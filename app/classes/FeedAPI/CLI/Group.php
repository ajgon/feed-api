...
class Group extends Base
{
    ...
    /**
     * ./feedapi group detach
     * Removes given feed from given group.
     *
     * @return null
     */
    public function detach()
    {
        $feed = new Feed();
        $group_id = $this->getGroupIDFromUser();
        $feed_id = $feed->getFeedIDFromUser('group', $group_id);

        if ($group_id > 0 && $feed_id > 0) {
            $res = \ORM::for_table('feeds_groups')->where('group_id', $group_id)->where('feed_id', $feed_id)->delete_many();
        }
    }
    ...
    public function help() {
        ...
        echo "  attach           - displays list of groups, then list of feeds. If both group\n".
             ...
             "                     selected group\n";
        echo "  detach           - removes given feed from given group\n";
